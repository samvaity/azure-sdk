 ...
 * @returns {{ byLanguage: { language: string, reviewer: string }[], assignees: string[] }}
 */
function resolveAssignments({ issueBody, issueNumber, approversConfig }) {
  ...
  return { byLanguage, assignees: [...assigneeSet] };
}
 ...
 *
 * @returns {Promise<{ byLanguage: { language: string, reviewer: string }[], assigned: string[], skipped: boolean }>}
 */
export default async function assignReviewers({
  ...
  if (newAssignees.length === 0) {
    ...
    return { byLanguage, assigned: [], skipped: true };
  }
  ...
  return { byLanguage, assigned: newAssignees, skipped: false };
}
